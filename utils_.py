...
def preprocess(path):
    ...
    numPeds = np.unique(data[2, :])

    return data, numPeds

def get_traj_like_nuscenes(data, numPeds):
    '''
       reshape data format from [filename, ped_ID, ...]
       to pedestrian_num * [ped_ID, filename, ...]
       '''

    traj_data = []

    for pedIndex in tqdm(numPeds):
        print(pedIndex)
        traj = []
        for i in range(len(data[2])):
            # and data[0][i] % 12 == 0
            if data[2][i] == pedIndex:
                traj.append([data[2][i], data[1][i], data[3][i], data[4][i]])
...
def get_obs_pred_like_nuscenes(data, observed_frame_num, predicting_frame_num):
    """"
    ...
    for pedIndex in tqdm(range(len(data))):
...
def location_input_nuscenes(obs, observed_frame_num):
    "get observed 3d location of pedestrians"

    location_input = []
    for pedIndex in range(len(obs)):
        person_pedIndex = []
        for i in range(observed_frame_num):
            person_pedIndex.append(
                [obs[pedIndex][i][3], obs[pedIndex][i][4]])
        person_pedIndex = np.reshape(person_pedIndex, [observed_frame_num, 2])

        location_input.append(person_pedIndex)

    location_input = np.reshape(location_input, [len(obs), observed_frame_num, 2])

    return location_input


def location_nuscenes(data, frame_num):
    "get 3d location of pedestrians"

    location_input = []
    for pedIndex in range(len(data)):
        person_pedIndex = []
        for i in range(frame_num):
            person_pedIndex.append([float(data[pedIndex][i][2]), float(data[pedIndex][i][3])])

        person_pedIndex = np.reshape(person_pedIndex, [frame_num, 2])

        location_input.append(person_pedIndex)

    location_input = np.reshape(location_input, [len(data), frame_num, 2])
...
def get_raw_data_nuscenes(path, observed_frame_num, predicting_frame_num):

    raw_data, numPeds = preprocess(path)
    data = get_traj_like_nuscenes(raw_data, numPeds)
    print(len(data))
    obs, pred = get_obs_pred_like_nuscenes(data, observed_frame_num, predicting_frame_num)
...
def rotate_via_numpy(traj, radians):
    """Use numpy to build a rotation matrix and take the dot product."""

    new_traj = []
    c, s = np.cos(radians), np.sin(radians)
    j = np.matrix([[c, s], [-s, c]])
    for ped in range(traj.shape[0]):
        for f in range(traj.shape[1]):
            m = np.dot(j, traj[ped, f, :])
            new_traj.append(m)

    new_traj = np.reshape(new_traj, (traj.shape[0], traj.shape[1], 2))
    return new_traj
